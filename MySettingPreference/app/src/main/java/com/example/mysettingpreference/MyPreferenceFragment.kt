...
class MyPreferenceFragment : PreferenceFragmentCompat(),
    ...
    private fun setSummaries() {
        ...
        if (sh != null) { namePreference.summary = sh.getString(NAME, DEFAULT_VALUE) }
        if (sh != null) { emailPreference.summary = sh.getString(EMAIL, DEFAULT_VALUE) }
        if (sh != null) { agePreference.summary = sh.getString(AGE, DEFAULT_VALUE) }
        if (sh != null) { phonePreference.summary = sh.getString(PHONE, DEFAULT_VALUE) }
        if (sh != null) { isLoveMuPreference.isChecked = sh.getBoolean(LOVE, false) }
    }
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        ...
        if (key == LOVE) { isLoveMuPreference.isChecked = sharedPreferences?.getBoolean(LOVE, false) as Boolean }
    }
}